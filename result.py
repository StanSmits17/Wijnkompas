...
def show_result():
    ...
    st.table(cluster_df['Druivensoort'])
    ...
    if st.button("Send result"):
        html_content = f"<html><body><p>Here are your top 5 recommended grape varieties:</p>{match_df.to_html(index=False)}<br><p>Similar grapes:</p>{'<br>'.join(cluster_df['Druivensoort'])}<br><img src='cid:plot' alt='Graph'></body></html>"
